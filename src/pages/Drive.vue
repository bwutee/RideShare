<template>
  <v-container>
    <div>
      ...
      <instructions details="Become a driver" />
  ...
  data: function () {
    return {
      ...
      rules: {
        ...
        licenseNumber: [
            (val) => val.length == 8 || "Lincese number must be length of 8",
        ],
        licenseState: [
            (val)=> /NY|IN/.test(val) || "Sorry, only IN or NY available for now"
        ]
      },
    };
  },
